...
export const recognize = (img: File, setter: React.Dispatch<number>, processSetter: React.Dispatch<number>) => {
  Tesseract.recognize(
    img,
    'eng',
    { logger: m => processSetter(m.progress*100) }
  ).then(({ data : { text }}) => {
...
const parseScoreInfo = (score: string) => {
  const [original, average, standardDeviation] = (score.match(/(\d*\.?\d*)/g) || []).filter((x) => !!x);
  return {
...
/** pass 과목을 필터링하는 함수 */
const filterPassSubject = (parsedArr: any[]) => {
...
const getOrderGradeWithStandardDeviationScore = (score: number) => {
  ...
  if (isBetween(1.76, 3.00, fixedScore)) return 1;
  ...
  if (isBetween(-3.00, -1.76,  fixedScore)) return 9;
  return 9;
}

/** 석차등급이 없는 과목의 석차등급을 구하여 할당해주는 함수 */
const convertToOrderGrade = (x: any) => {
  const { original, average, standardDeviation } = parseScoreInfo(x[1]);
  const numberGrade = (original - average)/standardDeviation
  x[3] = getOrderGradeWithStandardDeviationScore(numberGrade);
  return x;
}

const calculate = (arr: any[]) => {
  /** P 과목 제외  */
  const filterPassArr = filterPassSubject(arr);

  /** 석차등급이 없는 경우의 로직  */
  const hasNotOrderGrade = (([,,,numberGrade]: any) => !numberGrade);

  filterPassArr.forEach((arr, index) => {
    /** 석차 등급이 없는 경우 (성취평가제 교과인 경우, ex. A, B, C) */
    if (hasNotOrderGrade(arr)) {
      /** 해당 과목의 석차등급을 구하여 할당해준다. (표준화 점수 사용하여 환산) */
      filterPassArr[index] = convertToOrderGrade(arr)
    }
  })

  return calculateWithOrderGrade(filterPassArr);
}

/** 석차등급과 교과별 이수단위수를 사용하여 평균 등급 확인 */
const calculateWithOrderGrade = (parsedArr: any[]) => {
